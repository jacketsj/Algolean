...
def placementInstruction : RandomOpenInstruction signature → OpenInstruction signature
  | .machine (.core instruction) => .core instruction
  | .machine (.randBit _ next) => .core (.jump next)
  | .call op input output next => .call op input output next

def placement (client : RandomOpenProgram signature) : OpenProgram signature :=
  client.map placementInstruction

@[simp] theorem placement_length (client : RandomOpenProgram signature) :
    (placement client).length = client.length := by simp [placement]

theorem placement_valid (client : RandomOpenProgram signature) (valid : client.Valid) :
  ...
  have below := valid pc source sourceFetch target (by
    ...
    | machine machine => cases machine <;>
        simpa [placementInstruction, OpenInstruction.successors,
          RandomOpenInstruction.successors, RandomBit.Instruction.successors,
...
def clientInstruction (client : RandomOpenProgram signature)
    (environment : ImplementationEnvironment signature) (configuration : Linker.Configuration)
    (pc : Nat) : RandomOpenInstruction signature → RandomBit.Instruction
...
def clientCode (client : RandomOpenProgram signature)
    (environment : ImplementationEnvironment signature) (configuration : Linker.Configuration) :
    RandomBit.Program :=
  client.zipIdx.map fun (instruction, pc) ↦
    clientInstruction client environment configuration pc instruction

@[simp] theorem clientCode_length (client : RandomOpenProgram signature)
    (environment : ImplementationEnvironment signature) (configuration : Linker.Configuration) :
    (clientCode client environment configuration).length = client.length := by
  simp [clientCode]

def deterministicArtifact (client : RandomOpenProgram signature)
    (environment : ImplementationEnvironment signature) (configuration : Linker.Configuration) :
...
def link (client : RandomOpenProgram signature)
    (environment : ImplementationEnvironment signature) (configuration : Linker.Configuration) :
    RandomBit.Program :=
...
theorem deterministic_length_le (client : RandomOpenProgram signature)
    (environment : ImplementationEnvironment signature) (configuration : Linker.Configuration) :
...
theorem link_length (client : RandomOpenProgram signature)
    (environment : ImplementationEnvironment signature) (configuration : Linker.Configuration) :
...
theorem link_fetch_client (client : RandomOpenProgram signature)
    (environment : ImplementationEnvironment signature) (configuration : Linker.Configuration)
...
theorem link_fetch_tail (client : RandomOpenProgram signature)
    (environment : ImplementationEnvironment signature) (configuration : Linker.Configuration)
...
theorem link_valid (client : RandomOpenProgram signature)
    (environment : ImplementationEnvironment signature) (configuration : Linker.Configuration)
    ...
    | machine machine =>
        ...
        have sourceSuccessor : target ∈
          ...
          cases machine <;> simpa [source, clientInstruction, placementInstruction,
            Linker.clientInstruction, RandomBit.Instruction.successors,
            Instruction.successors] using successor
...
theorem bodyMaterialized (client : RandomOpenProgram signature)
    (environment : ImplementationEnvironment signature) (configuration : Linker.Configuration)
